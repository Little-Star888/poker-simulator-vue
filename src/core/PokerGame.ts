...
export class PokerGame {
  ...
  reset(settings: Settings, forceDealerIndex: number | null = null): void {
    ...
    if (forceDealerIndex !== null) {
      ...
      if (roleIndex !== -1) {
        ...
        console.warn(
          `Role ${this.settings.p1Role} is not valid for ${playerCount} players. Assigning roles randomly.`,
        );
        this.dealerIndex = Math.floor(Math.random() * playerCount);
      }
    }
  ...
  dealFlop(): void {
    if (this.usePresetCommunity) {
      if (
        !this.presetCards ||
        !this.presetCards.flop ||
        this.presetCards.flop.length !== 3
      ) {
  ...
  dealTurnOrRiver(): void {
    if (this.usePresetCommunity) {
      if (this.communityCards.length === 3) {
        if (
          !this.presetCards ||
          !this.presetCards.turn ||
          this.presetCards.turn.length !== 1
        ) {
        ...
        if (
          !this.presetCards ||
          !this.presetCards.river ||
          this.presetCards.river.length !== 1
        ) {
  ...
  startNewRound(roundName: GameRound): void {
    ...
    if (roundName === "preflop") {
      this.currentPlayerIndex =
        playerCount === 2 ? this.sbIndex : (this.bbIndex + 1) % playerCount;
    ...
    while (
      ...
      console.log(
        `[startNewRound] Skipping player ${this.players[this.currentPlayerIndex].id} (folded: ${this.players[this.currentPlayerIndex].isFolded}, allIn: ${this.players[this.currentPlayerIndex].isAllIn})`,
      );
    ...
    switch (action) {
      ...
      case "CHECK":
        if (player.bet < this.highestBet) {
          throw new Error(
            `Cannot check, must call ${this.highestBet} or fold.`,
          );
        }
        break;
      ...
      case "RAISE": {
        ...
        if (raiseAmount < this.lastRaiseAmount && !isAllIn) {
          throw new Error(
            `Raise must be at least ${this.lastRaiseAmount}. Your raise of ${raiseAmount} is too small.`,
          );
        }
      ...
      case "BET": {
        if (this.highestBet > 0) {
          throw new Error(
            "Cannot BET, there is already a bet. Action should be RAISE.",
          );
        }
  ...
  moveToNextPlayer(): void {
    console.log(
      `[moveToNextPlayer] Starting. Current index: ${this.currentPlayerIndex}`,
    );
    let attempts = 0;
    ...
    do {
      ...
      console.log(
        `[moveToNextPlayer] Trying index ${this.currentPlayerIndex} (${currentPlayer.id}). isFolded: ${currentPlayer.isFolded}`,
      );
      if (!currentPlayer.isFolded && !currentPlayer.isAllIn) {
        console.log(
          `[moveToNextPlayer] Found active player: ${currentPlayer.id}. Returning.`,
        );
        return;
      }
      attempts++;
